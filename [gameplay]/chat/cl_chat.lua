...
local function refreshCommands()
    if GetRegisteredCommands then
        ...
        for _, command in ipairs(registeredCommands) do
            if IsAceAllowed(('command.%s'):format(command.name)) and
                command.name ~= 'toggleChat' then
...
local chatHideState = kvpEntry and tonumber(kvpEntry) or
                          CHAT_HIDE_STATES.SHOW_WHEN_ACTIVE
local isFirstHide = true

if not isRDR then
    if RegisterKeyMapping then
        RegisterKeyMapping('toggleChat', 'Ouvrir le chat', 'keyboard', 'l')
    end
